...
# Carga ubicaciones soportadas desde el archivo JSON
def load_supported_locations():
    # Obtiene el directorio donde se encuentra este archivo
    base_path = os.path.dirname(__file__)
    # Construye la ruta completa al archivo supported_locations.json
    json_file = os.path.join(base_path, 'supported_locations.json')
    
    # Abre y lee el JSON con las ubicaciones soportadas
    with open(json_file, 'r') as f:
        return json.load(f)

# Guarda en memoria las ubicaciones soportadas (aeropuertos y ciudades)
supported_locations = load_supported_locations()


class TravelProvider(BaseProvider):
    def __init__(self, faker_instance):
        # Usa la instancia de Faker proporcionada
    ...
    def flight_lookup(self, departure_city, destination_city, num_options=3):
        ...
        """
        # Valida que la ciudad de salida esté soportada
        ...
        # Valida que la ciudad de destino esté soportada
        ...
        for _ in range(num_options):
            ...
            # Genera precios aleatorios por clase
            ...
            # Agrega la opción de vuelo al listado
            flights.append({
                'airline': airline,
        ...
        # Retorna respuesta con código y listado de opciones
    ...
    def hotel_lookup(self, city, num_options=3):
        ...
        """
        # Valida que la ciudad esté soportada
        ...
        hotels = []
        for _ in range(num_options):
            # Elige un nombre de hotel aleatorio
            hotel_name = random.choice(['Hilton', 'Marriott', 'Hyatt', 'Holiday Inn', 'Sheraton'])

            # Genera fechas de check-in y check-out
            ...
            # Agrega la opción de hotel al listado
            hotels.append({
...
# Inicializa Faker de forma global
fake = Faker()

# Inicializa TravelProvider con la instancia de Faker
